 ...
 *
 * Creates auth users and populates all tables with mock data.
 *
 * Prerequisites:
 *   1. Run schema.sql + migration-subjects.sql in Supabase SQL Editor first
 *   2. Get your service role key from Supabase Dashboard > Settings > API
 *   3. Run: SUPABASE_SERVICE_ROLE_KEY=your_key npx tsx supabase/seed-users.ts
 */
...
interface StudentSeed {
  ...
  address: string;
  grades: GradeRow[];
  reflections: { semester: string; text: string; target: string; teacherNote: string }[];
}

const studentSeeds: StudentSeed[] = [
  {
    email: "0012345678@learntrack.local",
    password: "siswa123",
    name: "Adi Nugroho",
    gender: "Male",
    ...
    reflections: [
      { semester: "Sem 1 (Grade 10)", text: "Saya perlu meningkatkan matematika. Akan belajar lebih giat semester depan.", target: "Mencapai 85 di matematika.", teacherNote: "Terus semangat Adi! Fokus di aljabar." },
      { semester: "Sem 2 (Grade 10)", text: "Matematika meningkat tapi fisika perlu perhatian lebih.", target: "Meningkatkan fisika ke 87.", teacherNote: "" },
    ],
  },
  {
    email: "0087654321@learntrack.local",
    password: "siswa123",
    name: "Siti Rahayu",
    gender: "Female",
    school: "SMA Negeri 1 Sragen",
    ...
    reflections: [
      { semester: "Sem 1 (Grade 10)", text: "Semester berjalan baik. Perlu menjaga konsistensi.", target: "Tetap di atas 88 di semua mapel.", teacherNote: "Performa luar biasa! Pertahankan." },
    ],
  },
  {
    email: "0099887766@learntrack.local",
    password: "siswa123",
    name: "Bima Saputra",
    gender: "Male",
    school: "SMA Negeri 1 Sragen",
...
const teacherSeeds = [
  {
    ...
    subject: "Mathematics",
    phone: "081234567890",
    teacherEmail: "budi.santoso@sman1sragen.sch.id",
  },
  {
    email: "ani.widyastuti@sman1sragen.sch.id",
    password: "guru123",
    name: "Ani Widyastuti, M.Pd.",
    gender: "Female",
    school: "SMA Negeri 1 Sragen",
    schoolYear: "2024/2025",
    ...
    phone: "082345678901",
    teacherEmail: "ani.widyastuti@sman1sragen.sch.id",
  },
];
...
async function seed() {
  ...
  for (const t of teacherSeeds) {
    ...
    if (error) {
      if (error.message.includes("already been registered")) {
        ...
        if (existing) { teacherIds[t.classHandled] = existing.id; console.log(`  -> Already exists: ${existing.id}`); continue; }
      }
      console.error(`  -> Error: ${error.message}`); continue;
    }
    ...
    await supabase.from("teachers").insert({ id: userId, nip: t.nip, class_handled: t.classHandled, subject: t.subject, phone: t.phone, email: t.teacherEmail });
  }
  ...
  for (const s of studentSeeds) {
    ...
    if (error) {
      if (error.message.includes("already been registered")) { console.log(`  -> Already exists, skipping`); continue; }
      console.error(`  -> Error: ${error.message}`); continue;
    }
    const userId = data.user.id;
    console.log(`  -> Created: ${userId}`);
    ...
    await supabase.from("students").insert({ id: userId, nisn: s.nisn, birth_date: s.birthDate, class: s.class, address: s.address });

    const teacherId = teacherIds[s.class] || null;
    for (const g of s.grades) {
      await supabase.from("grades").insert({ student_id: userId, uploaded_by: teacherId, ...g });
    }
    for (const r of s.reflections) {
      await supabase.from("reflections").insert({
        student_id: userId, semester: r.semester, text: r.text, target: r.target,
        teacher_note: r.teacherNote || null, noted_by: r.teacherNote ? teacherId : null,
      });
    }
  }
  ...
  console.log("\nSeed complete!");
  console.log("\nDemo credentials:");
  console.log("  Student: 0012345678 / siswa123");
